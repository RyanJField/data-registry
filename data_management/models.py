...
class Object(BaseModel):
    issues = models.ManyToManyField(Issue, related_name='object_issues')


class ObjectComponent(BaseModel):
    name = models.CharField(max_length=CHAR_FIELD_LENGTH, null=False, blank=False)
    issues = models.ManyToManyField(Issue, related_name='component_issues')
...
class StorageRoot(BaseModel):
    ...
    name = models.CharField(max_length=CHAR_FIELD_LENGTH, null=False, blank=False)
    root = models.URLField(null=False, blank=False)
    accessibility = models.SmallIntegerField(max_length=10, choices=CHOICES, default=PUBLIC)
...
class ExternalObject(BaseModel):
    ...
    original_store = models.ForeignKey(StorageLocation, on_delete=models.CASCADE, related_name='original_store_of')
...
class DataProduct(BaseModel):
    ...
    name = models.CharField(max_length=CHAR_FIELD_LENGTH, null=False, blank=False)
    versions = semantic_version.django_fields.VersionField()
